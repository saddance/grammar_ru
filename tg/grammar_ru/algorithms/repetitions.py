from .architecture import NlpAlgorithm, ErrorTypes
...
class RepetitionsAlgorithm(NlpAlgorithm):
    ...
    def compare(self, merge_df, word_df):
        ...
        if cdf.shape[0] == 0:
            return None
        cdf = cdf.feed(
            fluq.add_ordering_column("word_id", ("another_id", False), "order")
        )
    ...
    def _run_inner(self, db: DataBundle, index: pd.Index):
        if "pymorphy" not in db.data_frames and (
            self.allow_normal_form_check or self.allow_tikhonov_check
        ):  # TODO: this belongs to NlpAlgorithm
        ...
        ref_df = (
            df.loc[err]
            .repetition_reference.to_frame()
            .merge(
                df.set_index("word_id").word,
                left_on="repetition_reference",
                right_index=True,
            )
            .word
        )
        if "algorithm" in df.columns:
            df.loc[err, NlpAlgorithm.Hint] = (
                ...
                + ref_df
                + "` located "
                + (df.loc[err, "word_id"] - df.loc[err, "repetition_reference"]).astype(
                    str
                )
            )
        df[NlpAlgorithm.ErrorType] = ErrorTypes.Stylistic
        return df

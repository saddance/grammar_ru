...
from tg.grammar_ru.components.yandex_delivery.training_logs import (
    S3TrainingLogsLoader,
    TrainingLogsViewer,
)
...
def plot_loss(metrics, title=""):
    plt.plot(TrainingLogsViewer.get_metric_by_job(metrics, "loss"), label="loss")
    plt.title(title)
    plt.legend()
    plt.show()
...
def get_worst_words_sents(
    result_df, src, true_label: int, pred_label: int, worst_words_cnt: int
):
    one_inst_another = result_df[
        (result_df.true_label == true_label) & (result_df.pred_label == pred_label)
    ]
    thrsh = (
        one_inst_another[f"predicted_label_{pred_label}"]
        .sort_values(ascending=False)
        .head(worst_words_cnt)
        .min()
    )
    ...
    worst_words = (
        src[src.word_id.isin(worst_mistakes_scores.word_id)][
...
def get_best_words_sents(result_df, src, pred_label: int, words_cnt: int):
    ...
    thrsh = (
        correct_df[f"predicted_label_{pred_label}"]
        .sort_values(ascending=False)
        .head(words_cnt)
        .min()
    )
    best_scores = correct_df[correct_df[f"predicted_label_{pred_label}"] >= thrsh]
    best_words = (src[src.word_id.isin(best_scores.word_id)])[
        ["word_id", "sentence_id", "word"]
    ][:words_cnt]
...
def get_training_results(bucket, job_name, project_name):
    ...
    unzipped_folder = (
        Loc.root_path / "temp" / "training_results" / f"{tasks[0]}.unzipped"
    )
